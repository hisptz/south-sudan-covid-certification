...
export class AnalyticsService {
  ...
  loadEnrollements(): Observable<any> {
    const url =
      ...
      'dimension=CTdzCeTbYay.yCWkkKtr6vd&' + 'dimension=CTdzCeTbYay.eydwReGQiUk&' +
  ...
  loadOrgUnitDataWithAncestors(orgUnitId) {
      const url = this.apiUrl + `organisationUnits/${orgUnitId}.json?fields=id,name,level,ancestors[id,name,%20level]`;
      return this.httpClient.get(url);
  }
}
